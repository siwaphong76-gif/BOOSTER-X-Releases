...
function Invoke-DownloadFile {
    ...
    $headers = @{
        'User-Agent' = 'BOOSTER-X-Installer/1.7.6'
...
try {
    ...
    try {
        $rawManifest = Invoke-RestMethod -Uri $manifestRequestUrl -UseBasicParsing -Headers @{'User-Agent'='BOOSTER-X-Installer/1.7.6';'Cache-Control'='no-cache'} -TimeoutSec 30
    }
    catch {
