...
function Invoke-DownloadFile {
    ...
    $headers = @{
        'User-Agent' = 'BOOSTER-X-Installer/1.5.1'
...
try {
    ...
    try {
        $rawManifest = Invoke-RestMethod -Uri $manifestRequestUrl -UseBasicParsing -Headers @{'User-Agent'='BOOSTER-X-Installer/1.5.1';'Cache-Control'='no-cache'} -TimeoutSec 30
    }
